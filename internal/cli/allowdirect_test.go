...
// ONLY RFC1918 + link-local ranges and rejecting public IPs / hostnames /
// malformed values LOUDLY at startup. This file is the parse+validate contract;
// it stands up no jail and needs no podman.

// runArgs prefixes the required subcommand + proxy so each case names only the
// --allow-direct values it exercises.
...
// IP and CIDR values, with and without a :port, in BOTH `--flag value` and
// `--flag=value` forms, parse into the typed allowlist on Command.
func TestParse_AllowDirect_PrivateEntriesBothForms(t *testing.T) {
	cmd, err := cli.ParseWithEnv(runArgs(
		"--allow-direct", "192.168.1.150:8080", // IP + port, space form
		"--allow-direct=10.0.0.0/24:443", // CIDR + port, equals form
		"--allow-direct", "172.16.5.5",   // bare IP, no port
		"--allow-direct=169.254.0.0/16", // link-local CIDR, no port
	), noEnv)
	if err != nil {
...
func TestParse_AllowDirect_RejectsPublicIP(t *testing.T) {
	_, err := cli.ParseWithEnv(runArgs("--allow-direct", "8.8.8.8"), noEnv)
	if err == nil {
...
func TestParse_AllowDirect_RejectsPublicCIDR(t *testing.T) {
	_, err := cli.ParseWithEnv(runArgs("--allow-direct", "1.2.3.0/24"), noEnv)
	if err == nil {
...
func TestParse_AllowDirect_RejectsPartlyPublicCIDR(t *testing.T) {
	...
	_, err := cli.ParseWithEnv(runArgs("--allow-direct", "10.0.0.0/7"), noEnv)
	if err == nil {
...
func TestParse_AllowDirect_RejectsHostname(t *testing.T) {
	_, err := cli.ParseWithEnv(runArgs("--allow-direct", "llama.local:8080"), noEnv)
	if err == nil {
...
func TestParse_AllowDirect_RejectsMalformed(t *testing.T) {
	for _, v := range []string{"not-an-ip", "192.168.1.999", "10.0.0.0/33", "192.168.1.1/"} {
...
func TestParse_AllowDirect_MissingValueRejected(t *testing.T) {
	_, err := cli.ParseWithEnv([]string{"run", "--proxy", "socks5h://h:1", "--allow-direct"}, noEnv)
	if err == nil {
		t.Fatal("--allow-direct with no value accepted; want rejection")
	}
	if !strings.Contains(err.Error(), "--allow-direct") {
