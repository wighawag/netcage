...
- **The RFC1918 drops + the accept rules are added ONLY for a NON-EMPTY allowlist.** IMPORTANT: today's `nftRuleset` has NO RFC1918 drop rules at all - the current default jail's fail-closed comes from the TUN-only route (anything not excluded goes to the TUN -> proxy), not from nft dropping private ranges. So the accept + RFC1918-drop block is emitted ONLY when the allowlist is non-empty; an empty allowlist must add NOTHING to `nftRuleset` (see the next bullet). Adding unconditional RFC1918 drops would break the empty==today invariant and the existing `TestNftRuleset_*` unit tests.
- **Off by default == today's jail:** an EMPTY allowlist must produce byte-identical `SidecarRunArgs` + `nftRuleset` to the current strict jail (no extra excluded routes, no accept rules, no RFC1918 drops), so the existing forced-egress / teardown / leak tests do not regress. The existing `TestNftRuleset_*` and `TestSidecarRunArgs_*` unit tests are the guard: they must pass UNCHANGED with an empty allowlist.
...
- [ ] Tests written FIRST: with an allowlist, `SidecarRunArgs` includes each allowed net in `TUN_EXCLUDED_ROUTES` (alongside the proxy reachback), and `nftRuleset` emits the `ip daddr <host> tcp dport <port> accept` rule(s) BEFORE the drops, with the RFC1918-range drops after (both the accept AND the RFC1918 drops present ONLY when the allowlist is non-empty). With an EMPTY allowlist, both outputs are BYTE-IDENTICAL to today, no added excluded routes, no accept rules, and NO RFC1918 drops (which do not exist today); the existing `TestNftRuleset_*` / `TestSidecarRunArgs_*` unit tests pass UNCHANGED.
...
>
> FIRST, check against current reality: confirm `SidecarRunArgs` still builds `TUN_EXCLUDED_ROUTES=<reachback>/32` and that `nftRuleset` today emits the loopback-DNS accept + `meta l4proto udp drop` + (only under `ProxyOnHostLoopback`) the reachback narrowing, and NOTE that it has NO RFC1918 drop rules today (fail-closed is the TUN-only route). Confirm the CLI task landed the validated allowlist on `Command`/`Config` as assumed. If any moved, reconcile (route to needs-attention) rather than building on a stale premise. The empty-allowlist path must leave `nftRuleset`/`SidecarRunArgs` output byte-identical to today; add the accept + RFC1918-drop block ONLY for a non-empty allowlist.
>
