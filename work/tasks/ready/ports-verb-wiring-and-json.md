...
- Resolve the named container to a netcage-managed run via the `netcage.managed` label, REUSING the same resolver `forward` uses (`resolveManagedTool` -> run id -> `sidecarNameFor`); refuse a non-netcage or unknown container loudly, and refuse a stopped jail loudly (nothing to enumerate), same as forward.
