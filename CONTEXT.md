...
- **`netcage start` (jail-aware resume)** — the verb that RESUMES a kept, netcage-managed tool container with its full jail restored (ADR-0011, prd stories 7+9): REVIVE the existing sidecar (baked firewall re-applies on start, then VERIFIED like the run path), re-exec the DNS forwarder, re-enter the tool with state intact. It is NOT a pass-through verb (`internal/manage`): it lives in the jail package (`jail.Start`), carries a `--proxy`/`--allow`, and RECONCILES them against the container's BAKED config (read from the sidecar's create-time `PROXY`/`TUN_EXCLUDED_ROUTES`/`EXTRA_COMMANDS` env). **Same config ⇒ revive; different `--proxy`/`--allow` ⇒ REFUSE** (`ErrJailConfigChanged`, state-preserving), never a silently-stale jail. Attaches by default (`-a`; `-it` ⇒ `-ai` raw shell); honours the kept/ephemeral split like `run`.
