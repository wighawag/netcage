...
Likewise **ICMP from the jail is dropped**: a `ping` (or any raw ICMP) to an off-box address gets no reply, so no packet carrying the real source IP ever reaches the network (the jail confines non-TCP; ICMP does not ride the TUN-to-SOCKS path, forced traffic is tun2socks-relayed TCP). Caveat: dropping ICMP forgoes classic ICMP path-MTU discovery, and netcage deliberately does **not** tune PMTU (it sets no `net.ipv4.tcp_mtu_probing`), because the forced path is tun2socks-relayed TCP and no motivating tool needs raw ICMP; this is revisited only if a real tool's PMTU breaks.
